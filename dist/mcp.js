 ...
 */
export function applyMcps(servers, projectDir, agents) {
    const claudeServers = {};
...
function toClaudeShape(name, s) {
    ...
    if (!s.command)
        fail(`mcp server "${name}" needs either "command" or "url"`);
    return {
        command: s.command,
...
function writeClaudeMcp(projectDir, servers) {
...
export function upsertCodexMcp(name, s) {
    if (s.url) {
    ...
    const section = new RegExp(String.raw `(^|\n)\[mcp_servers\.(?:${esc}|"${esc}")(?:\.[^\]\n]+)?\][^]*?(?=\n\[|$)`, "g");
