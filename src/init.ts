...
export function resolveAgents(ai: string): string[] {
  if (ai === "both") return ["claude", "codex"];
  if (ai === "claude" || ai === "codex") return [ai];
...
export function detectAgents(projectDir: string, fallback: string): string[] {
  const found: string[] = [];
  if (fs.existsSync(path.join(projectDir, ".claude"))) found.push("claude");
...
export function specifyCmd(cfg: Config): string[] {
  if (which("specify")) return ["specify"];
  if (which("uvx")) return ["uvx", "--from", cfg.defaults.specKit, "specify"];
  fail("neither 'specify' nor 'uvx' found — install uv first: https://docs.astral.sh/uv/");
}

export async function cmdInit(opts: InitOptions, cfg: Config): Promise<void> {
  ...
  if (opts.noGit) initCmd.push("--no-git");
  if (opts.force) initCmd.push("--force");
  if (opts.ignoreAgentTools) initCmd.push("--ignore-agent-tools");
  run(initCmd);
  for (const agent of rest) {
    ...
    run(
      [...specify, "init", "--here", "--force", "--ai", agent, "--script", script, "--no-git", "--ignore-agent-tools"],
      { cwd: projectDir },
    );
  }

  // 2. extensions
  const extensions: Record<string, string> = opts.noExt ? {} : { ...cfg.extensions };
  for (const src of opts.extraExtensions) extensions[src] = src;
  ...
  if (agents.includes("claude")) note("  Claude Code: open the project and run /speckit-grill etc.");
  if (agents.includes("codex")) note("  Codex: prompts are available as /speckit-* commands");
}
