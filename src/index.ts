...
function parse(argv: string[], spec: FlagSpec): { flags: Record<string, any>; pos: string[] } {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith("--")) {
      ...
      if (!kind) fail(`unknown flag --${name} (see csc --help)`);
      if (kind === "bool") {
        flags[name] = true;
      } else {
        const v = inline ?? argv[++i];
        if (v === undefined) fail(`--${name} expects a value`);
        if (kind === "multi") (flags[name] ??= []).push(v);
        else flags[name] = v;
      }
    } else {
      pos.push(a);
...
function requireProjectDir(dir: string): string {
  ...
  switch (cmd) {
    ...
    case "ext": {
      ...
      if (sub === "list" || sub === undefined) {
        for (const [name, src] of Object.entries(cfg.extensions)) note(`${name}\t${src}`);
        if (!Object.keys(cfg.extensions).length) note("(no extensions configured)");
        return;
      }
      if (sub === "add") {
        const [name, src] = [argv[2], argv[3]];
        if (!name || !src) fail("usage: csc ext add <name> <url|path>");
      ...
      if (sub === "remove") {
        ...
        if (!(name in cfg.extensions)) fail(`no extension named "${name}" in config`);
    ...
    case "mcp": {
      ...
      if (sub === "list" || sub === undefined) {
        ...
        if (!Object.keys(cfg.mcp).length) note("(no MCP servers configured)");
        return;
      }
      if (sub === "add") {
        ...
        if (!flags.command && !flags.url) fail("mcp add needs --command or --url");
        const server: McpServer = {};
        if (flags.command) server.command = flags.command;
        if (flags.arg?.length) server.args = flags.arg;
        if (flags.env?.length) {
          server.env = Object.fromEntries(flags.env.map((e: string) => parseKv(e, "--env")));
        }
        if (flags.url) server.url = flags.url;
        if (flags.header?.length) {
          server.headers = Object.fromEntries(
            flags.header.map((h: string) => parseKv(h, "--header")),
          );
        }
        if (flags.agents) {
          server.agents = flags.agents.split(",").map((a: string) => a.trim());
          for (const a of server.agents!) {
            if (a !== "claude" && a !== "codex") fail(`--agents accepts claude,codex (got "${a}")`);
          }
        }
        cfg.mcp[name] = server;
        saveConfig(cfg);
      ...
      if (sub === "remove") {
        ...
        if (!(name in cfg.mcp)) fail(`no MCP server named "${name}" in config`);
        delete cfg.mcp[name];
        saveConfig(cfg);
      ...
      if (sub === "apply") {
        ...
        const projectDir = requireProjectDir(flags.project ?? process.cwd());
        if (!Object.keys(cfg.mcp).length) fail("no MCP servers configured — add one with: csc mcp add");
        const agents = flags.ai
          ? resolveAgents(flags.ai)
          : detectAgents(projectDir, cfg.defaults.ai);
    ...
    case "config": {
      ...
      if (sub === "get") {
        ...
        if (v === undefined) fail(`no such key: ${key}`);
      ...
      if (sub === "set") {
        const key = argv[2];
        const raw = argv[3];
        if (!key || raw === undefined) fail('usage: csc config set <dotted.key> <value> (JSON or string)');
    ...
    case "self": {
      if (argv[1] !== "update") fail("usage: csc self update");
      const cfg = loadConfig();
